...
export const DEFAULT_OG_URL = 'https://roomies.dk';
 ...
 * @param {string} [img='https://roomies.dk/pics/opengraph.webp'] - Billede til social deling
 */
export function updateMetaTags(
    title,
    desc,
    url = window.location.href, // Default til den nuværende URL
    img = 'https://roomies.dk/pics/opengraph.webp' // Dit standard OpenGraph billede
) {
...
/**
 * Find a housing document in window.housings.
 * By default looks for a match on '_id', but can be configured to check other fields like 'created_by'.
 * Checks if data is currently fetching and waits if necessary.
 *
 * @param {string} target_id - The value to search for.
 * @param {string} [field='_id'] - The property key to match against (defaults to '_id').
 * @returns {Promise<any|null>} The matching housing document, or null if not found.
 */
export async function getHousingById(target_id, field = '_id', options = {}) {
    const timeoutMs = Number.isFinite(options.timeoutMs) ? options.timeoutMs : 12000;

    // Helper for synchronous lookup
    const findSync = () => {
        if (!window.housings) {
            return null;
        }
        for (const h of window.housings) {
            // Compare as strings to ensure safety (e.g. ObjectId vs String)
            if (h && String(h[field]) === String(target_id)) {
                return h;
            }
        }
        return null;
    ...
    // 1. Try immediate lookup (Fast path - great for when navigating inside the SPA)
    let match = findSync();
    if (match) return match;

    // 2. If not found, and a fetch is in progress, await it (Slow path - handles fresh page loads)
    if (window.housingFetchPromise) {
        try {
            await waitForPromiseWithTimeout(
                window.housingFetchPromise,
                timeoutMs,
                `Timed out waiting for global housing data (${field}=${target_id})`
            );
            // Try lookup again after fetch completes
            match = findSync();
            if (match) return match;
        } catch (e) {
            console.error(`Error awaiting background fetch in getHousingById for ${field}=${target_id}:`, e);
        }
    }
